...
- **SonarCloud issue backlog — 5 remaining** in `src/providers/llm.py`. `turn()` dispatch / content-block translators at complexity 18-43. Ships in its own focused PR with per-provider unit coverage — these refactors are easy to get subtly wrong because each provider has a different wire format. Hotspots today: `_messages_to_gemini_contents` (31), `_messages_to_openai` (32), `_messages_to_ollama` (43), plus the two `turn()` methods at 18-19.
