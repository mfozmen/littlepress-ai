...
| TBD | `chore/mailmap-historical-email` | Non-destructive canonicalisation of the maintainer's historical author identity. New `.mailmap` at repo root maps the old work email (`mehmet.fahri@mayadem.com`) → canonical personal email (`mehmetfahriozmen@gmail.com`) so `git log` / `git shortlog` / GitHub's commit-list UI display the canonical identity everywhere without rewriting any SHAs. Chose this over a `git filter-repo` rewrite because the non-destructive path is honest, cheap, and has zero blast radius for clones / forks / open PRs. GitHub-noreply identity on squash-merge commits left alone by design (it's GitHub's own platform identity). |
