...
| TBD | `feat/deterministic-ingestion` | First sub-project of the "AI-only-for-judgment" refactor. `src/ingestion.py` runs OCR + sentinel classification (``<BLANK>`` / ``<TEXT>`` / ``<MIXED>``) on every image-only page in deterministic Python *before* the agent's first turn. REPL hooks into the load flow; the agent greeting no longer tells the agent to re-run the pipeline itself. Transcribe tool stays registered for post-render re-OCR requests. Metadata / cover / back-cover deterministic collection follows in a later sub-PR. |
