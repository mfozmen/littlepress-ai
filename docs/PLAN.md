...
- **Don't overwrite previous renders.** Today `render_book` and `/render` both write `.book-gen/output/<slug>.pdf` unconditionally — rendering the same draft twice silently clobbers the earlier PDF. Fix: keep a "latest" copy at `<slug>.pdf` but version each render alongside it (e.g. `<slug>-2026-04-16T1045.pdf` or a monotonically-incrementing `-v2`, `-v3`). The booklet gets the same treatment. The agent's message should report both the stable filename and the versioned one so the user can compare or roll back.
- **Collect user PDFs in `.book-gen/input/`.** Today the draft PDF lives wherever the user dropped it (Downloads, Desktop, …) and `Draft.source_pdf` / memory store that absolute path. If the user moves or deletes the original, rerunning `littlepress` can't match memory → they lose their session. Fix: on first load, copy the PDF into `.book-gen/input/<slug>.pdf` and retarget `source_pdf` at the copy. Memory then keys off a path we control; the user's Downloads folder can be cleaned up without breaking anything. `/load` and the CLI bootstrap both go through the same helper.
