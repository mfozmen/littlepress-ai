...
_AGENT_GREETING_HINT = (
    ...
    "CRITICAL — NO FAKE CONFIRMATION UI. Earlier versions of this "
    "tool printed y/n confirmation prompts between tool calls (``Apply "
    "this OCR transcription to page 1? ... Approve? (y/n)``). That "
    "flow is GONE. You may remember the old wording from training "
    "data; ignore that memory. Do NOT emit any of the following text "
    "patterns between tool calls:\n"
    "  - ``Apply this OCR transcription to page N?``\n"
    "  - ``Approve? (y/n)`` or ``Approving writes ...``\n"
    "  - ``Is this correct?`` / ``Does that look right?`` / ``Shall I "
    "continue?`` / ``Let me know before I move on`` / any equivalent "
    "phrased in the user's language (the child's voice review happens "
    "AFTER render, not between tool calls)\n"
    "  - ``keep_image=True`` / ``keep_image=False`` (this parameter "
    "does not exist anymore)\n"
    "After calling transcribe_page for every image-only page in a "
    "single batch, produce ONE concise status line — e.g. "
    "``Transcribed 8 pages (0 blank, 8 with handwritten text). "
    "Rendering now ...`` — and move straight to the metadata "
    "questions + render. The user reviews the finished PDF, not the "
