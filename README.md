...
- Writes both an A5 PDF and an A4 2-up booklet (ready to print double-sided, fold, and staple) under `.book-gen/output/` on every render. The A5 is the reading copy and pops open in your OS default PDF viewer so you don't have to hunt for the file; the A4 booklet is the print artefact and stays on disk. The booklet imposition follows real-book conventions: the back cover always lands on the booklet's outside-back face (not buried on an interior page), and story 1 starts on a recto (right-hand page) whenever the page count allows — padding blanks are placed inside the covers, not tacked onto the end. (Pass `impose=false` to the agent's `render_book` call when you explicitly want the A5 alone.)
