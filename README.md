...
- Runs an agent conversation (Claude by default; Gemini fully supported, including tool use; OpenAI / Ollama still in the picker but pending SDK wiring). The agent replies in whatever language you type in. Gemini's free tier (1.5k requests/day at time of writing) lets you try Littlepress without a credit card.
...
- Three cover templates: `full-bleed` (drawing covers the whole page, title on a translucent band — the default, best for dramatic illustrations), `framed` (letterboxed drawing with the title above and author below), and `poster` (type-only cover with no drawing, for books whose child-author didn't make a cover illustration). The agent picks one when setting the cover — guided by the `select-cover-template` skill — and you can change it any time through the conversation.
