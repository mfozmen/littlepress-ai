...
Turn a child's picture-book **draft PDF** (scanned handwriting + drawings) into a print-ready A5 book — and the matching A4 imposed booklet ready to fold and staple, generated alongside on every render. The goal is simple: **a child should feel like a real author** — their original words are preserved end-to-end.
...
4. It renders a polished A5 picture book PDF, plus the matching A4 imposed booklet for home printing — both produced on every render.
...
- Optional AI cover generation when the child didn't draw a cover (or wants a different one). When OpenAI is configured as the image provider, the agent can offer `generate_cover_illustration`: it proposes a prompt + a quality tier (low ≈ $0.02, medium ≈ $0.06, high ≈ $0.25 on OpenAI's gpt-image-1 at 1024x1536 portrait — approximate, check the OpenAI pricing page), shows both in a y/n confirmation so nothing is spent without your say-so, and on approval saves the PNG under `.book-gen/images/` and wires it up as the cover. The image provider is decoupled from the chat provider via `/image-model` — you can run Claude / Gemini / Ollama as the chat agent and still have OpenAI handle image generation. Chat=OpenAI sessions auto-wire the image provider from the chat key, so single-provider OpenAI users don't need `/image-model`.
- Optional AI page illustrations (`generate_page_illustration`). The natural follow-up to the Samsung-Notes duplicate-text fix: once `transcribe_page` clears the screenshot-image on a page, the page renders as text-only with no picture. When OpenAI is configured as the image provider (via the chat-OpenAI auto-wire or an explicit `/image-model openai`), the agent can propose a fresh illustration per page — same pricing tiers as the cover tool, same y/n confirm-with-price gate, same preserve-child-voice wording ("describe the scene in your own words — don't paraphrase the child's text"). Set an optional `layout` to switch the page off text-only in the same call.
...
- **Duplicate title-header strip.** Once metadata is collected, Littlepress checks whether the first non-hidden page's transcribed text starts with a header that closely matches the book title (casefold + diacritic-fold + sequence similarity, ≥ 0.8; up to 3 leading lines are tried, so a wrapped multi-line title drops as one unit). If it does — the common Samsung-Notes case where the child wrote the title at the top of their first physical page — the duplicate header is dropped so the title only appears on the cover, not again at the start of the first interior page. Edge case: when the page consisted of ONLY the header (a dedicated title page before the story starts), the page is auto-hidden so the rendered book doesn't include a blank interior page; in the post-render review turn, telling the agent "bring page N back" calls `restore_page(page=N)` to un-hide it (you supply the page number; the original drawing re-attaches automatically). Genuinely different headings (e.g. an `Author's note`) score below the threshold and stay put.
...
- Writes exactly two PDFs per render under `.book-gen/output/`: the A5 reading copy (`<slug>.pdf`) and the A4 2-up booklet (`<slug>_A4_booklet.pdf`, ready to print double-sided, fold, and staple). The A5 pops open in your OS default PDF viewer so you don't have to hunt for the file; the A4 stays on disk for printing. Re-rendering overwrites the same two files in place — no versioned snapshots cluttering the directory. Pass `impose=false` to the agent's `render_book` call when you explicitly want the A5 alone.
- Auto-housekeeping on every render: orphan images from earlier `generate_*_illustration` retries are dropped from `.book-gen/images/`. The stable `<slug>.pdf` / `<slug>_A4_booklet.pdf` pointers, your input PDFs, and referenced cover/page images are never touched. Run it manually with `/prune` (add `--dry-run` to preview).
