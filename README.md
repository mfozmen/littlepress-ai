...
- Two cover templates: `full-bleed` (drawing covers the whole page, title on a translucent band — the default, best for dramatic illustrations) and `framed` (letterboxed drawing with the title above and author below). The agent picks one when setting the cover, and you can change it any time through the conversation.
