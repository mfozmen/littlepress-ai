...
- Runs an agent conversation (Claude by default; Gemini and GPT (OpenAI) fully supported, including tool use; Ollama still in the picker but pending local-model wiring). The agent replies in whatever language you type in. Gemini's free tier (1.5k requests/day at time of writing) lets you try Littlepress without a credit card.
