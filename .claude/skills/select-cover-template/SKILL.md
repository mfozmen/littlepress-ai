...
## Goal state (future, more templates)

The three templates are a coarse start. A richer vocabulary is planned (see `docs/PLAN.md`):

- `portrait-frame` — illustration inside a decorative border with margins.
- `title-band-top` — variant of framed with a coloured panel behind the title.
- `spine-wrap` — drawing spans front + spine + back for the A4 imposed booklet.

Until those ship, the three named templates are the vocabulary; this skill should still compute the fit reasoning above, not just pattern-match a name.
